import random
import time
from aiogram import Router, F
from aiogram.filters import CommandStart
from aiogram.types import Message, CallbackQuery
from aiogram.fsm.context import FSMContext

from templates.money_bot.database import MoneyDB
from templates.money_bot.keyboards import main_menu_kb, admin_main_kb, captcha_kb, share_ref_link_kb
from templates.money_bot.handlers.subscription import check_subscription, send_subscription_message, send_subscription_callback
from templates.money_bot.states import UserStates

THROTTLING = {}

def is_throttled(user_id: int) -> bool:
    now = time.time()
    if user_id in THROTTLING:
        if now - THROTTLING[user_id] < 1.0: # 1 second limit
            return True
    THROTTLING[user_id] = now
    return False


def create_user_router(money_db: MoneyDB, admin_id: int) -> Router:
    router = Router()

    @router.message(CommandStart())
    async def cmd_start(message: Message, state: FSMContext):
        if is_throttled(message.from_user.id): return
        
        await state.clear()
        user_id = message.from_user.id
        text = message.text or ""

        # Referral logic
        referrer_id = None
        if " " in text:
            arg = text.split(" ", 1)[1].strip()
            if arg.startswith("ref_"):
                try:
                    referrer_id = int(arg[4:])
                    if referrer_id == user_id:
                        referrer_id = None
                except ValueError:
                    pass

        is_new = await money_db.add_user(
            telegram_id=user_id, 
            username=message.from_user.username, 
            referred_by=referrer_id
        )

        user = await money_db.get_user(user_id)
        
        # Check if user is verified (Anti-Bot)
        if not user or not user["is_verified"]:
            num1 = random.randint(1, 10)
            num2 = random.randint(1, 10)
            correct_ans = num1 + num2
            await message.answer(
                f"🤖 <b>Anti-Bot tekshiruvi!</b>\n\n"
                f"Siz robot emasmisiz? Misolni yeching:\n\n"
                f"<b>{num1} + {num2} = ?</b>",
                reply_markup=captcha_kb(correct_ans),
                parse_mode="HTML"
            )
            return

        # Check sub
        if not await check_subscription(message.bot, user_id, money_db):
            await send_subscription_message(message, money_db)
            return

        await message.answer(
            f"👋 Assalomu alaykum, <b>{message.from_user.full_name}</b>!\n\n"
            f"💸 <b>Pul ishlash botiga xush kelibsiz!</b>\n\n"
            f"Do'stlaringizni taklif qiling va haqiqiy pul ishlang!",
            reply_markup=main_menu_kb(),
            parse_mode="HTML"
        )

    @router.callback_query(F.data.startswith("captcha:"))
    async def process_captcha(callback: CallbackQuery):
        res = callback.data.split(":")[1]
        user_id = callback.from_user.id
        
        if res == "pass":
            await callback.message.delete()
            await money_db.verify_user(user_id)
            
            # Reward referrer if exists
            user = await money_db.get_user(user_id)
            referrer_id = user["referred_by"] if user else None
            
            if referrer_id:
                ref_bonus = int(await money_db.get_setting("ref_bonus"))
                await money_db.update_balance(referrer_id, ref_bonus)
                try:
                    await callback.bot.send_message(
                        referrer_id,
                        f"🎉 <b>Yangi referal!</b>\n\n"
                        f"👤 {callback.from_user.full_name} sizning havolangiz orqali qo'shildi va tasdiqdan o'tdi.\n"
                        f"💰 <b>+{ref_bonus:,} so'm</b> balansga qo'shildi!",
                        parse_mode="HTML"
                    )
                except Exception:
                    pass

            if not await check_subscription(callback.bot, user_id, money_db):
                await send_subscription_callback(callback, money_db)
                return

            await callback.message.answer(
                f"✅ <b>Tasdiqlandi!</b>\n\n"
                f"Botdan to'liq foydalanishingiz mumkin.",
                reply_markup=main_menu_kb(),
                parse_mode="HTML"
            )
        else:
            await callback.answer("❌ Noto'g'ri javob!", show_alert=True)

    @router.callback_query(F.data == "check_sub")
    async def check_sub_callback(callback: CallbackQuery):
        if is_throttled(callback.from_user.id): return
        
        if not await check_subscription(callback.bot, callback.from_user.id, money_db):
            await callback.answer("Hali hamma kanallarga obuna bo'lmapsiz!", show_alert=True)
            return
        
        await callback.message.delete()
        await callback.message.answer(
            f"✅ <b>Obuna tasdiqlandi!</b>\n\n"
            f"Endi botdan to'liq foydalanishingiz mumkin.",
            reply_markup=main_menu_kb(),
            parse_mode="HTML"
        )

    @router.message(F.text == "💰 Balansim")
    async def show_balance(message: Message):
        if is_throttled(message.from_user.id): return
        if not await check_subscription(message.bot, message.from_user.id, money_db):
            await send_subscription_message(message, money_db)
            return

        user_id = message.from_user.id
        user = await money_db.get_user(user_id)
        
        await message.answer(
            f"💰 <b>Sizning balansingiz</b>\n\n"
            f"💵 Joriy balans: <b>{user['balance']:,} so'm</b>\n"
            f"📈 Jami ishlangan: <b>{user.get('total_earned', 0):,} so'm</b>",
            parse_mode="HTML"
        )

    @router.message(F.text == "💸 Pul ishlash (Referral)")
    async def earn_money(message: Message):
        if is_throttled(message.from_user.id): return
        if not await check_subscription(message.bot, message.from_user.id, money_db):
            await send_subscription_message(message, money_db)
            return

        user_id = message.from_user.id
        bot_me = await message.bot.get_me()
        ref_link = f"https://t.me/{bot_me.username}?start=ref_{user_id}"
        
        ref_count = await money_db.get_referral_count(user_id)
        ref_bonus = int(await money_db.get_setting("ref_bonus"))
        user = await money_db.get_user(user_id)
        ref_photo = await money_db.get_setting("ref_photo_id")

        text = (
            f"🔗 <b>Do'stlarni taklif qiling!</b>\n\n"
            f"Har bir taklif qilingan do'stingiz uchun <b>{ref_bonus:,} so'm</b> olasiz!\n\n"
            f"📊 <b>Sizning statistika:</b>\n"
            f"👥 Takliflar: <b>{ref_count}</b> ta\n"
            f"💰 Balans: <b>{user['balance']:,} so'm</b>\n\n"
            f"👇 <b>Sizning taklif havolangiz:</b>\n"
            f"<code>{ref_link}</code>\n\n"
            f"Buni nusxalab do'stlaringizga yuboring!"
        )

        if ref_photo:
            await message.answer_photo(
                photo=ref_photo,
                caption=text,
                reply_markup=share_ref_link_kb(ref_link),
                parse_mode="HTML"
            )
        else:
            await message.answer(
                text,
                reply_markup=share_ref_link_kb(ref_link),
                parse_mode="HTML"
            )

    @router.message(F.text == "💳 Pul yechish")
    async def withdraw_money(message: Message, state: FSMContext):
        if is_throttled(message.from_user.id): return
        if not await check_subscription(message.bot, message.from_user.id, money_db):
            await send_subscription_message(message, money_db)
            return

        user_id = message.from_user.id
        user = await money_db.get_user(user_id)
        min_withdraw = int(await money_db.get_setting("min_withdraw"))

        if user['balance'] < min_withdraw:
            await message.answer(
                f"❌ <b>Balans yetarli emas!</b>\n\n"
                f"Sizning balans: <b>{user['balance']:,} so'm</b>\n"
                f"Minimal yechish: <b>{min_withdraw:,} so'm</b>\n\n"
                f"Yana do'stlaringizni taklif qiling!",
                parse_mode="HTML"
            )
            return

        await state.update_data(withdraw_amount=user['balance'])
        await message.answer(
            f"💸 <b>Pul yechish</b>\n\n"
            f"Sizning balans: <b>{user['balance']:,} so'm</b>\n\n"
            f"Iltimos, pulni qabul qilib oluvchi <b>Karta raqamingizni (yoki telefon raqam)</b> yuboring:",
            parse_mode="HTML"
        )
        await state.set_state(UserStates.waiting_withdraw_details)

    @router.message(UserStates.waiting_withdraw_details)
    async def withdraw_details(message: Message, state: FSMContext):
        if is_throttled(message.from_user.id): return
        details = message.text
        data = await state.get_data()
        amount = data['withdraw_amount']
        user_id = message.from_user.id
        user = message.from_user

        # Remove stars from user balance
        await money_db.update_balance(user_id, -amount)

        # Notify admin
        from templates.money_bot.keyboards import payout_approve_kb
        admin_text = (
            f"💸 <b>Yangi pul yechish so'rovi!</b>\n\n"
            f"👤 User: {user.full_name}\n"
            f"🆔 ID: <code>{user_id}</code>\n"
            f"💰 Miqdor: <b>{amount:,} so'm</b>\n\n"
            f"📝 Qabul qiluvchi (Karta/Nomer): \n<code>{details}</code>\n\n"
            f"To'lovni amalga oshirgach 'To'landi' tugmasini bosing."
        )
        try:
            await message.bot.send_message(
                admin_id,
                admin_text,
                reply_markup=payout_approve_kb(user_id, amount),
                parse_mode="HTML"
            )
        except Exception:
            pass

        await message.answer(
            f"✅ <b>So'rov adminga yuborildi!</b>\n\n"
            f"Miqdor: <b>{amount:,} so'm</b>\n"
            f"To'lov tasdiqlangach sizga xabar beramiz.",
            reply_markup=main_menu_kb(),
            parse_mode="HTML"
        )
        await state.clear()

    @router.message(F.text == "🏆 TOP Reyting")
    async def show_top(message: Message):
        if is_throttled(message.from_user.id): return
        top_users = await money_db.get_top_users(10)
        
        text = "🏆 <b>TOP 10 Eng ko'p pul ishlaganlar:</b>\n\n"
        medals = ["🥇", "🥈", "🥉"]
        
        for i, u in enumerate(top_users):
            medal = medals[i] if i < 3 else "🏅"
            username_display = f"@{u['username']}" if u['username'] else f"ID: {u['telegram_id']}"
            text += f"{medal} {username_display} — <b>{u['total_earned']:,} so'm</b>\n"
            
        await message.answer(text, parse_mode="HTML")

    @router.message(F.text == "📊 To'lovlar")
    async def show_payouts(message: Message):
        if is_throttled(message.from_user.id): return
        payout_channel = await money_db.get_setting("payout_channel_username")
        if payout_channel:
            await message.answer(
                f"📊 Bizning barcha to'lovlarimiz shu kanalda e'lon qilib boriladi:\n\n"
                f"👉 {payout_channel}"
            )
        else:
            await message.answer("📊 To'lovlar kanali hali sozlanmagan.")

    @router.message(F.text == "ℹ️ Qoidalari")
    async def show_faq(message: Message):
        if is_throttled(message.from_user.id): return
        ref_bonus = int(await money_db.get_setting("ref_bonus"))
        min_withdraw = int(await money_db.get_setting("min_withdraw"))
        
        await message.answer(
            f"ℹ️ <b>Qoidalari va shartlar</b>\n\n"
            f"1. <b>💸 Pul ishlash</b> tugmasini bosing va havolangizni oling.\n"
            f"2. Havolani do'stlaringizga yuboring.\n"
            f"3. Do'stingiz botga kirib, kanallarga a'zo bo'lgach va <b>tasdiqdan o'tgach</b> sizga bonus beriladi.\n"
            f"4. Har bir referal uchun <b>{ref_bonus:,} so'm</b> olasiz.\n"
            f"5. Balansingiz <b>{min_withdraw:,} so'm</b> ga yetganda yechib olishingiz mumkin.\n\n"
            f"⚠️ <i>Sun'iy nakrutka qilish qat'iyan man etiladi va to'lov qilinmaydi!</i>",
            parse_mode="HTML"
        )

    # Allow admin to switch back to admin menu if they act like a user
    @router.message(F.text == "👤 User rejimi")
    async def user_mode(message: Message):
        if message.from_user.id == admin_id:
            await message.answer("👤 User rejimidasiz. Admin panelga qaytish uchun /admin bosing.", reply_markup=main_menu_kb())

    return router
